...
- 本工具的快照之间大量文件通过 **NTFS 硬链接**共享同一份物理数据。**不要在备份目录中手动逐文件删除/移动/覆盖**，否则会改变硬链接引用计数，可能导致其他快照中的同名文件一并消失或出现不一致。
- **清理旧备份请使用 `recall prune`（或交互界面内置的清理功能）**。`prune` 按“快照目录”粒度处理，能避免手动操作带来的硬链接误删与结构破坏：
  ```bash
  recall prune "E:\Backups\Projects" --keep 5

* 资源管理器删除会移除一个硬链接（目录项），并减少链接计数；当删除到最后一个链接时，底层数据会被释放，且在支持 TRIM/UNMAP 的介质上可能被立即‘读零化’，导致无法恢复。
* 如确需人工操作（不推荐）：只按“快照目录”整体删除，并确保不是 `current` 指向的最新快照；严禁在快照目录内部逐文件删除。
  * `current`在windows上是符号链接，删除 current 通常只会删掉链接本身，目标快照目录仍在
  * 但如果用户进入 current 里做删除/修改，实际是在操作目标快照目录（因为路径解析会跟随 symlink）
